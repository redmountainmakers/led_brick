...
def checkPos(pos):
    print "current position: [%s, %s]" % (playerPos["y"], playerPos["x"])
...
def moveDec():
    ...
    if moveDirec == Direction.UP:
        print "up direction check is " + str(checkPos(up))
        if checkPos(up):
            moveUpdate()
            dMov = True
    elif moveDirec == Direction.DOWN:
        print "down direction check is " + str(checkPos(down))
        if checkPos(down):
            moveUpdate()
            dMov = True
    elif moveDirec == Direction.LEFT:
        print "left direction check is " + str(checkPos(left))
        if checkPos(left):
            moveUpdate()
            dMov = True
    elif moveDirec == Direction.RIGHT:
        print "right direction check is " + str(checkPos(right))
        if checkPos(right):
            moveUpdate()
            dMov = True
    ...
    if not dMov:
        ...
        if not dMov:
            for x, y in enumerate(poss):
                print "secondary direction check is " + str(checkPos(y))
                if checkPos(y):
                    if x == 0:
